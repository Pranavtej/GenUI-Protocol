import { Component, forwardRef, ViewEncapsulation, input, output, model } from '@angular/core';
...
export class GuipCheckbox implements ControlValueAccessor {
  id = input(`checkbox-${Math.random().toString(36).substr(2, 9)}`);
  label = input('');
  required = input(false);
  disabled = model(false);
  color = input('');
  checkChange = output<boolean>({alias: 'change'});
